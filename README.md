...
```sh
make docker-build docker-push IMG=<some-registry>/ekswatch-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/ekswatch-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/ekswatch-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/ekswatch-operator/<tag or branch>/dist/install.yaml
```
